import time, socket, os, uuid, mss, json, asyncio, threading, aiohttp, screenMediaSourceClass, pygame
...
import guiTransmitterDrawing
import clientGlobals
# Non GUI version of SimpleCast client to get basics working

global clientPsk, configServerList, serverPinAuthNeeded, serverAudioAllowed, willSendAudio, globalPcObject, monitorsFound
...
async def showAndSelectSelectableServers():
    global clientPsk, configServerList, monitorsFound
    ...
    arrayIndex = 0
    for listEntry in clientGlobals.foundServers:
...
async def startConnecting(serverIndex):
    ...
    combinedList = []
    for item in clientGlobals.foundServers:
...
def programStart():
    ...
    # Start PyGame Thread
    pyGameThread = threading.Thread(target=guiTransmitterDrawing.pyGameDrawingThread)
    pyGameThread.start()
    
    # Start broadcast receiver thread
    broadcastReceiveThread = threading.Thread(target=threadedBroadcastScan)
